...
describe(
  ...
  withCognitoSdk((Cognito) => {
    ...
    it("returns SOFTWARE_TOKEN_MFA challenge for a TOTP-only user", async () => {
      const client = Cognito();
      ...
      await client
        .verifySoftwareToken({
          ...
          UserCode: generate(assoc.SecretCode!),
        })
        .promise();
      await client
      ...
      expect(mfaAuth.ChallengeName).toEqual("SOFTWARE_TOKEN_MFA");
      expect(mfaAuth.Session).toBeDefined();
    });
  }),
);
